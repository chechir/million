...
##### RUN another xgboost!
print 'second xgbooooost'
preds_path_xgb = tools.dropbox() + '/million/cache/preds_xgb_bagged_{}.pkl'.format(n_bags_xgb)
calculate_preds_xgb = True
if calculate_preds_xgb:
    np.random.seed(seed)
    ...
    for i in range(n_bags_xgb):
        ...
        txgb_preds = model.predict(dtest) + txgb_preds
        #prepare for the next iteration
        df_bag, bag_targets = tools.delete_some_outliers(df_train, targets)
        dtrain = xgb.DMatrix(df_bag.values, bag_targets)
        num_boost_rounds = 155
        print(i, df_bag.shape)

    txgb_preds = txgb_preds / n_bags_xgb

    tools.write_pickle(txgb_preds, preds_path_xgb)
else:
    txgb_preds = tools.read_pickle(preds_path_xgb)

print( "\Second XGBoost predictions:" )
print( pd.DataFrame(txgb_preds).head() )
##### RUN CATBOOST
...
print( pd.DataFrame(sub_preds).head() )
...
xgb_pred1 = TXGB_WEIGHT*txgb_preds + (1-TXGB_WEIGHT)*xgb_pred1
...
submission.to_csv('sub_lgb1_xgb2_cat1_2percent_{}.csv'.format(datetime.now().strftime('%Y%m%d_%H%M%S')), index=False)
